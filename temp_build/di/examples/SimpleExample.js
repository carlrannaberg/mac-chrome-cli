...
const CONFIG_TOKEN = createServiceToken('ConfigurationService');
const CACHE_TOKEN = createServiceToken('CacheService');
export async function runSimpleExample() {
    ...
    container.registerSingleton(CONFIG_TOKEN, () => new ConfigurationService());
    // Register cache service (depends on configuration)
    container.registerSingleton(CACHE_TOKEN, async (serviceContainer) => {
